...
/*
   03-Sep-95 ADC Call to errorSet not supported (nor required)
   if (error != GOOD_DATE)
   {
       errorSet(DTSPLIT_BAD_DATE, dt);
   }
*/
   return (error);
 } /* dt_split() */
 ...
 *        1994-03-05 - logic to match month was incorrect if an invalid
 *                     month was passed.
...
{ 
  ...
  for (; (unsigned) N1 < strlen((char *)s); N1++) { 
...
{ 
  ...
  time(&Timer); 
  pTM = localtime(&Timer); 
  if (((2000+*year) - (pTM->tm_year+1900))
      <= ((pTM->tm_year+1900) - (1900+*year))) 
      *year = *year + 2000; 
  else 
    *year = *year + 1900; 
 
  /* Month */ 
  for (iMonth=1; (iMonth < 13) && (memcmp((sdt+7),Month[iMonth],3) != 0); 
       iMonth++) { 
  } 
  if (iMonth == 13) 
    return (1); 
...
{ 
  ...
  /* Month */ 
  for (iMonth=1; (iMonth < 13) && (memcmp((fdt+3),Month[iMonth],3) != 0); 
       iMonth++) { 
  } 
  if (iMonth == 13) 
    return (1); 
